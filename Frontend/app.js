...
document.addEventListener("DOMContentLoaded", async () => {
  await loadWorkouts()   // wait until workouts are fetched
  updateStats()          // now update with correct data
  loadTheme()
  setTodayDate()
  setupEventListeners()
})
...
async function handleFormSubmit(e) {
  ...
  // Reload from DB so frontend and backend stay in sync
  await loadWorkouts()
  updateStats()
...
// Storage (replaced with MongoDB fetch calls)
async function loadWorkouts() {
  const res = await fetch(API_URL)
  workouts = await res.json()
}
async function saveWorkouts() {
  // No need, handled by backend
}
...
function renderHistoryTable() {
  ...
  tbody.innerHTML = sorted.map(w => `
    <tr>
      ...
      <td>${w.notes || "-"}</td>
      <td><button class="btn btn-sm btn-danger" onclick="deleteWorkout('${w._id}')"><i class="bi bi-trash"></i></button></td>
    </tr>`).join("")
}
